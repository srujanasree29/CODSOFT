...
for i in range(len(key_matrix)):
    ...
    for j in range(len(key_matrix[i])):
        ...
        row=i+1, column=j, padx=5, pady=5, ipadx=25, ipady=5)
        btn_dict["btn_"+str(key_matrix[i][j])].configure(
                 bg='#4C4C4C',  # Set your desired background color here
                 fg='white')
